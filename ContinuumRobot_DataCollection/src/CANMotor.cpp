...
DWORD CANMotor::SetVelocity(signed short Speed[3]){
    DWORD dwRel;
    ...
    WrValue[0] = Speed[0]/3000*8192*ReductionRatio;
    WrValue[1] = Speed[1]/3000*8192*ReductionRatio;
    WrValue[2] = Speed[2]/3000*8192*ReductionRatio;    
...
double * CANMotor::GetPosition(){
    ...
    //Function code
    for(int i=0;i<3;i++){
        SendMotor[i].Data[0] =0x00;
        SendMotor[i].Data[1] =0x2A;
    }
    //Read position request command set
    for(int i=0;i<3;i++){
        SendMotor[i].Data[2] = 0xE8;SendMotor[i].Data[3] = 0x00;SendMotor[i].Data[4] = 0x00;
        SendMotor[i].Data[5] = 0xE9;SendMotor[i].Data[6] = 0x00;SendMotor[i].Data[7] = 0x00;
    }
    dwRel = Transmit(DeviceType, DeviceIndex, CANIndex,SendMotor,3);//3-SendData[0-2]
    ...
    if(dwRel!=0xFFFFFFFF){
        ...
        for(i=0;i<3;i++){// i<3 for three motors       
            ...
            Position[i] = Position_Encoder[i]*2*PI/Encoder_PPR/ReductionRatio;//Encoder data to radian(output shaft of retarder)
        }
    }
    else{
